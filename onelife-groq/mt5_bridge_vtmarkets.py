...
def get_filling_mode(symbol):
    ...
    # VT Markets prefers RETURN mode, then try others
...
def trade():
    ...
    req = {
        ...
        "magic":        MAGIC,
        "comment":      comment,
        "type_filling": mt5.ORDER_FILLING_IOC,  # VT Markets only accepts IOC
    }

    log.info(f"Sending order: {direction} {lot}lot {symbol} @ {price} SL={sl} TP={tp}")
